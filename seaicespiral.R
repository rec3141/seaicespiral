...
for (i in 1:(dim(icevolume)[1])) {
 if (i %% 3 != 0) {
    ...
    polar.plot(icevolume$vol[1:i], polar.pos=icevolume$icedeg[1:i], clockwise=T, start=-20, rp.type="s", point.symbols='.', point.col='blue', radial.lim=c(0,5,15,25,35),radial.labels=c(NA,5,15,25,35),labels=monthlabel,label.pos=monthdeg,show.radial.grid=F, add=T)
...
for (i in 1:100) {
  par(cex=2,cex.axis=1.2,cex.lab=1.2, cex.main=1.2)
	polar.plot(0, 0, clockwise=T, start=-20, rp.type="s", point.symbols='.', point.col='blue', radial.lim=c(0,5,15,25,35),radial.labels=c(NA,5,15,25,35),labels=monthlabel,label.pos=monthdeg,show.radial.grid=F, mar=c(4,2,4.5,3))
  rasterImage(img,-35,-35,35,35)
  polar.plot(icevolume$vol, polar.pos=icevolume$icedeg, clockwise=T, start=-20, rp.type="s", point.symbols='.', point.col='blue', radial.lim=c(0,5,15,25,35),radial.labels=c(NA,5,15,25,35),labels=monthlabel,label.pos=monthdeg,show.radial.grid=F, add=T)
